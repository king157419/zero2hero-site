...
function trySymlink(target: string, linkPath: string): void {
  try {
    ...
    // Windows: directory symlinks need Developer Mode/admin. Fall back to a
    // junction (no privilege required), then to a recursive copy.
    const absTarget = path.resolve(path.dirname(linkPath), target)
    try {
      fs.symlinkSync(absTarget, linkPath, "junction")
      return
    } catch (errJunction: unknown) {
      if ((errJunction as NodeJS.ErrnoException).code === "EEXIST") return
    }
    try {
      fs.cpSync(absTarget, linkPath, { recursive: true })
      return
    } catch (errCopy: unknown) {
      if ((errCopy as NodeJS.ErrnoException).code === "EEXIST") return
      throw errCopy
    }
  }
}
